...
public class PetClinicConfig {
    ...
    @Bean
    public SampleDataInitializer sampleDataInitializer(OwnerRepository ownerRepository, VetRepository vetRepository) {
        return new SampleDataInitializer(ownerRepository, vetRepository);
    }
    ...
    public static class SampleDataInitializer {
        ...
        public SampleDataInitializer(OwnerRepository ownerRepository, VetRepository vetRepository) {
            this.ownerRepository = ownerRepository;
            this.vetRepository = vetRepository;
        }

        @Bean
        public org.springframework.boot.CommandLineRunner loadSampleData() {
            return args -> {
                if (ownerRepository.count() == 0) {
                    ownerRepository.saveAll(sampleOwners());
                }
                if (vetRepository.count() == 0) {
                    vetRepository.saveAll(sampleVets());
                }
            };
        }
